...
class BuildChamber(Actor):
    ...
    def makeBuildChamber(self, length=200, width=200, height=200, unit="mm"):
        """
        Create a VTK actor representing a build chamber (box) with specified dimensions.
        Units can be 'mm' (default) or 'in' (inches).
        """
        print("Creating Build Chamber Actor with dimensions:", length, "x", width, "x", height, "and unit:", unit)
        if unit == "in":
            length *= 25.4
            width *= 25.4
            height *= 25.4
    ...
    def make_rectangle(self, width, height, thickness=0, unit="mm"):
        """
    ...
    def make_hollow_box_no_top(self, width, height, depth, wall_thickness=1.0, unit="mm"):
        """
    ...
    def contructNewPrinter(self, printerBed = []):
        ...
        self.actor = self.make_hollow_box_no_top(printerBed[0], printerBed[1], printerBed[2], wall_thickness=.005, unit="in")
